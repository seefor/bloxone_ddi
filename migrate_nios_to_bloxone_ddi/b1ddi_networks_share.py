...
logging.basicConfig(filename='networks-logs.log', level=logging.DEBUG)
...
with open('your_export.csv') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            if "network" == row[0]:
                line_count += 1
                x = ip_space_keys[row[31]]
                test = str(netmask_to_cidr(row[2]))
                if test != "32":
                    network = row[1] + '/' + test
                    nc_site = row[56]
                    payload = {'payload': {'address': network, 'space': x, 'tags':{'Site':nc_site}}}
                    obj = {'object': 'ipam/subnet'}
                    sif[line_count] =payload,obj
                else:
                    logging.error(f"Not Allowed - {row[1]}/{test}")
    print(f'Processed {line_count} networks.')
csv_file.close()
...
def b1ddi(payload, object):
    ...
    if "result" not in new_space_return:
        ...
        # print(payload, file = sample)
        # print(create_ip_space)
        # print(new_space_return, file = sample)
        return dict()
    sif = new_space_return["result"]
    ip_space_keys[sif['name']] = sif['id']
